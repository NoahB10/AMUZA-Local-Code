...
class PlotWindow(QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        # Create a custom toolbar for the graph editing buttons
        self.graph_toolbar = QToolBar("Graph Controls", self)
        self.addToolBar(Qt.LeftToolBarArea, self.graph_toolbar)

        # Manually add specific actions for navigation controls to the toolbar
        self.add_graph_controls()
    ...
    def add_graph_controls(self):
        """Add specific graph editing controls to the toolbar."""
        # Create actions similar to NavigationToolbar buttons for zoom, pan, and save
        zoom_action = QAction("Zoom", self)
        zoom_action.triggered.connect(lambda: self.canvas.toolbar.zoom())
        self.graph_toolbar.addAction(zoom_action)

        pan_action = QAction("Pan", self)
        pan_action.triggered.connect(lambda: self.canvas.toolbar.pan())
        self.graph_toolbar.addAction(pan_action)

        save_action = QAction("Save", self)
        save_action.triggered.connect(lambda: self.canvas.toolbar.save_figure())
        self.graph_toolbar.addAction(save_action)
...
class AMUZAGUI(QWidget):
    def __init__(self):
        ...
        filename_placeholder = QLabel("", self)
        filename_placeholder.setFixedHeight(220)  # Set a height to occupy the same space as the filename box
        self.command_layout.addWidget(filename_placeholder)
        ...
        self.plate_layout = QGridLayout()  # Initialize plate_layout for the well plate grid
        self.well_labels = {}
    ...
    def setup_well_plate(self):
        ...
        for i, row in enumerate(rows):
            for j, col in enumerate(columns):
                ...
                self.plate_layout.addWidget(label, i, j)  # Arrange labels in grid

        # Add Clear button at the bottom spanning the full width
        clear_button = QPushButton("Clear", self)
        clear_button.clicked.connect(self.clear_plate_selection)
        self.plate_layout.addWidget(clear_button, len(rows), 0, 1, len(columns))  # Span across all columns
    ...
    def enable_control_buttons(self):
        ...
        buttons = [self.start_datalogger_button, self.insert_button, self.eject_button, self.runplate_button, self.move_button]
